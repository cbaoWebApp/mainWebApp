...
if(isset($_POST['searchIcon']))
{
    ...
    // using concat mysql function
	 $select=$_POST['drop'];
	 if($select == 0){
		 $query = "SELECT * FROM fetches WHERE controlNum LIKE '%".$valueToSearch."%'";
		 $search_result = filterTable($query);
	 } else if($select == 1){
		 $query = "SELECT * FROM fetches WHERE permitNum LIKE '%".$valueToSearch."%'";
		 $search_result = filterTable($query);
	 } else if($select == 2){
		 $query = "SELECT * FROM fetches WHERE appNum LIKE '%".$valueToSearch."%'";
		 $search_result = filterTable($query);
	 } else if($select == 3){
		 $query = "SELECT * FROM fetches WHERE dateOfApp LIKE '%".$valueToSearch."%'";
		 $search_result = filterTable($query);
	 } else if($select == 4){
		 $query = "SELECT * FROM fetches WHERE status LIKE '%".$valueToSearch."%'";
		 $search_result = filterTable($query);
	 } else {
		 $query = "SELECT * FROM fetches";
		 $search_result = filterTable($query);
	 }
